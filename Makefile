...
DKMS_ROOT_PATH  := /usr/src/asus-aura-$(VERSION)
...
obj-m	:= $(patsubst %,%.o,asus-aura)
obj-ko	:= $(patsubst %,%.ko,asus-aura)
...
dkms-install:
	...
	cp $(CURDIR)/asus-aura.c $(DKMS_ROOT_PATH)

	sed -e "s/@CFLGS@/${MCFLAGS}/" \
	    -e "s/@VERSION@/$(VERSION)/" \
	    -i $(DKMS_ROOT_PATH)/dkms.conf

	dkms add asus-aura/$(VERSION)
	dkms build asus-aura/$(VERSION)
	dkms install asus-aura/$(VERSION)

dkms-uninstall:
	dkms remove asus-aura/$(VERSION) --all
